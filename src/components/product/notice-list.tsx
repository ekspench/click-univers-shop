...
export default function NoticeList() {
    const {data:{productId}}=useModalState();
  ...
  return (
    <div className="bg-white w-[450px] p-8">
      ...
      <div className="my-10">
        {notices&&notices.map((notice, noticeIdx) => (
          <div key={notice.id} className="flex w-full text-sm text-gray-500 space-x-4">
            ...
            <div className="border-t border-gray-200  w-full ">
              <h3 className="font-medium text-gray-900">{notice.customer.name}</h3>
              <p>
                <time dateTime={notice.created_at}>{dayjs(notice.created_at).format('DD/MM/YYYY')}</time>
              </p>
